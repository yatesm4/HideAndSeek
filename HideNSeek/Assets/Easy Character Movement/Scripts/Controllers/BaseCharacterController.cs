...
namespace ECM.Controllers
{
    ...
    public class BaseCharacterController : MonoBehaviour
    {
        ...
        {
            get { return movement.isGrounded ? _groundFriction : airFriction; }
        ...
        {
            get { return movement.isGrounded ? _brakingFriction : airFriction; }
        ...
        public void RotateTowards(Vector3 direction, bool onlyXZ = true)
        {
            movement.Rotate(direction, angularSpeed, onlyXZ);
        }
        ...
        public void RotateTowardsMoveDirection(bool onlyXZ = true)
        {
            RotateTowards(moveDirection, onlyXZ);
        }
        ...
        public void RotateTowardsVelocity(bool onlyXZ = true)
        {
            RotateTowards(movement.velocity, onlyXZ);
        }
        ...
        protected void Jump()
        {
            // If ground detection is paused (eg: jumping) AND vertical velocity is negative (eg: falling),
            // resume ground detection

            if (!movement.detectGround && movement.velocity.y < 0.0001f)
                movement.detectGround = true;
            ...
            // 'Pause' ground detection until 'falling' (vertical velocity < 0.0001f)

            movement.detectGround = false;
        }
        ...
        protected void Move()
        {
            ...
            // If using root motion and root motion is being applied (eg: grounded),
            // move without accel / decel, the animation takes full control

            var desiredVelocity = CalcDesiredVelocity();

            if (useRootMotion && applyRootMotion)
                movement.Move(desiredVelocity, !allowVerticalMovement);
            else
            {
                // Move with NEW physics behavior model (accel / decel and FRICTION)

                movement.Move(desiredVelocity, acceleration, deceleration, groundFriction,
                    useBrakingFriction ? brakingFriction : groundFriction, !allowVerticalMovement);
            }
        ...
        {
            allowVerticalMovement = _allowVerticalMovement;
